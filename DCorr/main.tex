...
\includegraphics[scale=0.6]{Fig1_draft.png}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[scale=1.0]{Fig1_ref.png}
\end{subfigure}
...
\end{figure}
The idea is that $\beta_{xy}>\beta_{yx}$ intuitively implies $Y$ ``drives'' $X$ more than $X$ ``drives'' $Y$.  Stated more formally, $\beta_{yx}>\beta_{xy}\Rightarrow\Delta>0$, which is reported as ``$Y$ CCM causes $X$''.  Likewise, $\beta_{xy}<\beta_{yx}$ implies $X$ CCM causes $Y$ and $\beta_{xy}=\beta_{yx}$ implies no CCM causality in the system.  It will be shown below that CCM causality is not necessarily related to causality as it is typically understood in physics.
