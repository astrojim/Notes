...
\end{eqnarray}
where $r_x,r_y,\beta_{xy},\beta_{yx}\in\mathbf{R}\ge 0$.  This pair of equations is a specific form of the two-dimensional coupled logistic map system, which is known to be chaotic \cite{A. L. LLOYD 1995}.
...
\begin{figure}[ht]
\label{fig:}
\caption{The directed correlation $\vec{D}$ of an RL circuit with $V(t)$ and $R(t)$ given by Eqn. \ref{eqn:vt} and Eqn. \ref{eqn:rt}, respectively, can be calculated for each of the three parameters pairs; i.e.\ $(V,I)$, $(R,I)$, and $(V,R)$.  The parameters used to produce these plots are explained in the text.  Notice that these results imply {\bf ????}, which agrees with the physicial "intuition" for this situation.}
\end{figure}
These directed correlations seem to imply CCM causality is at least representative of physical causality.  Notice, however, that changing the phase, $\omega$, of the resistance $R(t)$ can lead to situations that do not support the intuitive result of $R$ CCM causes $I$.  For example, consider $A=R_0=\omega=0.5$.  These values lead the the directed correlations in Figure \ref{fig:}.
\begin{figure}[ht]
\label{fig:}
\caption{The directed correlations $\vec{D}$ of Figure \ref{fig:} change dramatically when the phase between $R(t)$ and $V(t)$ is changed.  See the text for the parameters used to produce $\vec{D}$.}
\end{figure}
...
???
\end{equation}
where $\ldots$.  Such a scenario leads to the directed correlations shown in Figure \ref{fig:}.
\begin{figure}[ht]
\label{fig:}
\caption{The directed correlations of the RL circuit system with $V(t)$ given by Eqn. \ref{eqn:vt} and $R(t)$ given by Eqn.\ \ref{eqn:rttrig}.}
\end{figure}
...
\section{Direct Correlation and Non-Linear Data Sets}
The directed correlation does not need a physical causality interpretation to be useful.  It is can helpful in inferring relationships between time series from complicated, non-linear dynamics.  
...
\end{eqnarray}
where $r_x,r_y,r_z,\beta_{xy},\beta_{yx},\beta_{xz},\beta_{zx},\beta_{zy},\beta_{yz}\in\mathbf{R}\ge 0$.  There is no readily apparent physical intuiation for this system unlike the two population example (which was, likewise, non-linear).  For instance, if $\beta_{xz}=\beta_{}$ {\bf write this to sepcifically reflect the example you plan on plotting.}

\section{Conclusion}
{\bf Remember to state $E$ and $\tau$ for all the plots!}
\section{Appendix A: CCM Algorithm}
\label{sec:appA}
A description of this algrotihm is also available in \cite{suppref}.  It is elucidating to partition the CCM algorithm into five distinct (though related) steps:
\begin{enumerate}
...
\label{sec:appB}
The algrotihm we use for determining the embedding dimension $E$ can be outlined as follows:
\begin{enumerate}
