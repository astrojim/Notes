...
Consider evaluating the CCM correlations $C_{VI}$ and $C_{IV}$ for each $\Omega_v\in[0.01,2.0]$ in steps of $0.01$.  The CCM correlations are found using $E=2$ and $\tau=1$ and both are plotted in Figure \ref{fig:Av} (along with $\Delta$).
\begin{figure}[ht]
...
Finally, a comparison of PAI and CCM for the RL circuit example leads to similar conclusions.  The expectation is the $V$ drives $I$; thus, it is expected $\Delta^\prime = C_{V(VI)} - C_{I(IV)} > 0$ (which is what is observed).  See Figure \ref{fig:AvPAI}.
\begin{figure}[ht]
