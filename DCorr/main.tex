...
(c) & (d) \\[6pt]
\end{tabular}
\caption{These plots show the dependence of Eqn.\ \ref{eqn:delta} on $\beta_{xy}$ and $\beta_{yx}$.  See the text for details on how these plots were created along with a discussion of the interpretation of these plots in terms of CCM causality.}
\end{figure}
...
\caption{The determination of CCM causality in a system is dependent on the CCM parameters of embedding dimension $E$ and lag time step $\tau$.  This plot show the dependence of Eqn.\ \ref{eqn:delta} on $E$ and $\tau$ for the example system discussed in the text.  The white dots indicate $\Delta<0$.}
\end{figure}
...
(b)
\end{tabular}
\caption{The sign of $\Delta$, and thus the CCM causality, depends on $A$ and $B$. (a) $\Delta$ calculated as described in the text; (b) Two color version of (a) where black indicates $\Delta>1$ and white indicates $\Delta<1$.}
\label{fig:linearex1}
\end{figure}
...
\begin{equation}
V(t) = A_v \sin\left(t\right),
\end{equation}
where $A_v$ is the amplitude.

Consider evaluating the CCM correlations $C_{VI}$ and $C_{IV}$ for each $A_v\in[0.01,2.0] V$ in steps of $0.01 V$.  The CCM correlations are found using $E=3$ and $\tau=1$ and both are plotted in Figure \ref{fig:Av} (along with $\Delta$).
\begin{figure}[H]
...
(a) Correlation between $X$ and $X|Y$ \\[6pt]
%\includegraphics[scale=0.5]{RLCircuitPlots/SugFig3_YgX.eps} \\
(b) Correlation between $Y$ and $Y|X$ \\[6pt]
\end{tabular}
...
(a) Correlation between $X$ and $X|X$ \\[6pt]
%\includegraphics[scale=0.5]{RLCircuitPlots/SugFig3_XgXY.eps} \\
(b) Correlation between $X$ and $X|(XY)$ \\[6pt]
...
\caption{$\Delta^\prime$ as a function of $A$ and $B$.}
