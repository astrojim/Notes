...
The NASA OMNI data set consists of hourly-averaged time series measurements of several different space weather parameters from 1963 to present, collected from more than twenty different satellites, along with sunspot number and several different geomagnetic indices, including $D_{ST}$, collected from the NOAA National Geophysical Data Center \cite{omniweb.gsfc.nasa.gov/html/ow_data.html}.  The disturbance storm time, $D_{st}$, is a measure of geomagnetic activity \cite{IAGA Bulletin 40}.  The magnetic field measurements in OMNI data sets, i.e., $B_x$, $B_y$, and $B_z$ in GSE coordinates \cite{ M. A. Hapgood, "Space physics coordinate transformations: A user guide", in Planetary and Space Science, Vol. 40, No. 5, pp. 711-717, 1992}, are believed to be drivers of $D_{st}$ \cite{spaceweathertextbook}.

Let $\mathbf{P_{x}^L}=\{\{B_x(t^\prime)\},\{D_{st}(t^\prime)\}\;|\;t^\prime\in[t^\prime_0,L]\}$ be an ordered subset of the available time series data $\{\{B_x(t)\},\{D_{st}(t)\}\;|\;t\in[0,N]\}$ where $N$ is the number of hourly data points in the OMNI data set.  Let $\mathbf{P_{y}^L}$ and $\mathbf{P_{z}^L}$ be defined analogously.  If $\lambda_1^{x(y)(z)}$ is the weighted mean observed leaning for $\mathbf{P_{x(y)(z)}^L}$ using the 1-standard cause-effect assignment, then $n$ samples of $\mathbf{P_{x(y)(z)}^L}$, each with a different $t^\prime_0$,  would produce a set of leanings, $\{\lambda_1^{x(y)(z)}\}$ from which the causal inference could be drawn.
