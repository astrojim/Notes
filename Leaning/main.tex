...
The noise in this example did not affect the causal inference, i.e.\ $\langle \lambda_{y_t^\prime,x_{t-1}^\prime} \rangle_w \Rightarrow \mathbf{X}\xrightarrow{lean}\mathbf{Y}$ as expected.  However, it will be shown in the following sections that noise in the time series can lead to counterintuative causal inferences.  Thus, the tolerances described here will be used with all leaning calculations with the tolerances set to the estimated (or known if possible) noise levels for the individual time series.
