...
Most empirical data sets make observed leaning calculations either tedious, or prohibitively difficult, to do without a computational aid.  Consider the following algorithm to calculate the observed penchants of a time series pair $\bar{\mathbf{T}}=\{\mathbf{X},\mathbf{Y}\} = \{\{x_t\},\{y_t\}\}$ for some time step $t\in[0,L]$ where $L$ is the library length of both times series\footnote{The library length is assumed to be the same for both time series as a simplifying assumption.  The algorithm would not need to be altered significantly to account for slightly different library lengths.  However, if the two time series have dramatically different library lengths (e.g.\ $L_y = L_x/2$), then the algorithm would need to change; e.g. by sampling shorter time series from the longer one and running the equal-length leaning algorithm using the shorter time series and each of the sampled time series.}:
\begin{enumerate}
\item Initialize three counters, e.g.\ $N_{EC} = N_{C} = N_{E} = 0$
\item Initialize two iterators, e.g.\ $m=l$ and $n=l$
\item If $y_m = y_n \pm \delta_y$, then increment one of the counters, e.g.\ $N_E = N_E+1$ 
\item If $x_{m-l} = x_{n-l} \pm \delta_x$, then increment another of the counters, e.g.\ $N_C = N_C+1$
\item If both $y_m = y_n \pm \delta_y$ and $x_{m-l} = x_{n-l} \pm \delta_x$, then increment the last counters, e.g.\ $N_{EC} = N_{EC}+1$
\item Increment one of the iterators, e.g.\ $n = n+1$
\item Repeat steps 3-6 $\forall\;n\in[l,L]$
\item If $N_C = 0$, issue a warning and skip to the final step 
\item Calculate the estimated $P(E|C)$, e.g.\ $P_{EC} = \frac{N_{EC}}{N_C}$
\item Calculate the estimated $P(C)$, e.g.\ $P_{C} = \frac{N_{C}}{L-l}$
\item Calculate the estimated $P(E)$, e.g.\ $P_{E} = \frac{N_{E}}{L-l}$
\item Calculated and record the observed penchant, i.e.\ $\rho_{EC} = P_{EC}\left[1+P_C\left(1-P_C\right)^{-1}\right]-P_E\left(1-P_C\right)^{-1}$
\item Repeat step 1
\item Repeat steps 3-13 $\forall\;m\in[l,L]$
\end{enumerate}

This algorithm will be labeled {\sc PEN}$(\mathbf{X},\mathbf{Y},l,\delta_x,\delta_y)$, and it calculates the observed penchant $\rho_{y_t,x_{t-l}}\;\forall\;t\in[0,L]$.  There are $L-l$ penchants returned by this algorithm, and the arithmetic mean of this set of penchants corresponds to the weighted mean observed penchant calculation discussed in Section \ref{sec:wmean}.  

{\sc PEN}$(\mathbf{X},\mathbf{Y},l,\delta_x,\delta_y)$ depends on five parameters, the time series pair, $\{\mathbf{X},\mathbf{Y}\}$, the lag $l$ used to define the $l$-standard cause-effect assignment and the tolerances, $\delta_x$ and $\delta_y$, used to determine if two data points in the time series should be considered equal.  The lag is part of the definition of the penchant, but the tolerances have been explicitly added to the algorithm to deal with noise in the data.  The effect of these tolerance on the causal inference for a given times series set will be explored in the following sections.

Assume there exists a single-valued algorithm that returns the arithmetic mean of a set of numbers, i.e.\ {\sc MEAN}$(\cdot)$, then the weighted mean observed leaning algorithm, {\sc LEAN}$(\mathbf{X},\mathbf{Y},l,\delta_x,\delta_y)$, is defined as the difference between {\sc MEAN}$(${\sc PEN}$(\mathbf{X},\mathbf{Y},l,\delta_x,\delta_y))$ and {\sc MEAN}$(${\sc PEN}$(\mathbf{Y},\mathbf{X},l,\delta_x,\delta_y))$, in that order.   

A {\sc MATLAB} implementation of {\sc LEAN}$(\mathbf{X},\mathbf{Y},l,\delta_x,\delta_y)$ is used in all of the results shown below and is available at {\it github}$\backslash\ldots$.
...
\begin{figure*}[ht]
\begin{tabular}{cccc}
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A20B20.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A20B45.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A20B70.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A20B100.eps} \\
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A45B20.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A45B45.eps} &
...
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A70B20.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A70B45.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A70B70.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A70B100.eps} \\
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A100B20.eps} &
\includegraphics[scale=0.30]{NonlinearCyclicexample_Bdy_A100B45.eps} &
...
\end{figure*}
\begin{figure}[ht]
\includegraphics[scale=0.45]{NonlinearCyclicexample_Bxytol.eps}
\caption{(Color available online.) The leaning agrees with intuition for most noise levels when the y-tolerance is set to the noise level (i.e.\ $B=\delta_y$).}
\end{figure}
\begin{figure}[ht]
