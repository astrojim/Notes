...
Figure \ref{fig:empdata} shows three time series pairs with casual ``truths'' from the UCI Machine Learning Repository (MLR) \cite{}.  This data repository is a collection of data sets (not necessarily time series data) with known, intuitive, or assumed causal relationships meant for use in the testing of causal discovery algorithms in machine learning \cite{}.  

\begin{figure*}[ht]
\begin{tabular}{cc}
\includegraphics[scale=0.30]{EmpiricalData_p65X.eps} &
\includegraphics[scale=0.30]{EmpiricalData_p65Y.eps} \\
(a) Hang Seng Bank stock returns (X) & (b) HSBC Hldgs stock returns (Y) \\
\includegraphics[scale=0.30]{EmpiricalData_p87X.eps} &
\includegraphics[scale=0.30]{EmpiricalData_p87Y.eps} \\
(c) mean temperature (X) & (d) snow fall (Y) \\
\end{tabular}
\caption{(Color available online.) These times series data pairs are expected to have the causal relationships of $X\rightarrow Y$ for each pair of subplots [(a),(b)] and [(c),(d)], where $X$ or $Y$ is marked in parenthesis for each time series.}
\label{fig:empdata}
\end{figure*}
Figure \ref{fig:empdata}(a) and (b) are times series of the stock returns of Hang Seng Bank (the expected response) and HSBC Holdings (the expected driver) from January 4, 2000 to June 17, 2005.  HSBC Holdings is the expected response because it owns 60\% of Hang Seng Bank.  This data is pair 65 of the MLR \cite{}, and the data collection procedure is described there.

Figure \ref{fig:empdata}(c) and (d) are times series of the daily snowfall (the expected response) and mean temperature (the expected driver) from July 1 1972 to December 31 2009 at Whistler, BC, Canada (Latitude: 50$^\circ$04$^\prime$04.000$^{\prime\prime}$ N, Longitude: 122$^\circ$56$^\prime$50.000$^{\prime\prime}$ W, Elevation: 1835.00 meters).  From \cite{}, ``Common sense tells us that X [mean temperature] causes Y [snow fall] (with maybe very small feedback of Y on X). Confounders are present (e.g., day of the year).''  This data is pair 87 of the MLR \cite{}, and the data collection procedure is described there.

As stated previously, the primary difficulty in using the leaning for exploratory causal analysis is the determination of the cause-effect assignment and tolerance domains.  The above data is meant only to illustrate the use of the leanings, so while a thorough analysis of the noise in the system should precede the leaning calculations, such a step is avoided here for brevity.  
...
Table \ref{tab:emp} shows the weighted mean observed leaning, $\lambda_l^{xy}$, using the $1$-standard cause-effect assignment with $l\in[0,15]$ and using $\sigma_x$ and $\sigma_y$ estimated in the manner described above.  The first two times series pairs in the table, $\mathbf{\tau}_1$ and $\mathbf{\tau}_2$, are shown in Figure \ref{fig:empdata}.  The last two time series pairs in the table, $\mathbf{\tau}_3$ and $\mathbf{\tau}_4$, are similar to $
\mathbf{\tau}_2$.  $\mathbf{\tau}_3$ is times series of the stock returns of Cheung Kong (the expected response) and Hutchison (the expected driver) from January 4, 2000 to June 17, 2005.  Cheung Kong is the expected response because it owns 50\% of Hutchison.  This data is pair 66 of the MLR \cite{}.  $\mathbf{\tau}_4$ is times series of the stock returns of Sun Hung Kai Properties (the expected response) and Cheung kong (the expected driver) from January 4, 2000 to June 17, 2005.  Sun Hung Kai Properties is the expected response because ``Sun Hung Kai Prop. is a typical stock in the Hang Seng Property subindex'' \cite{}.  This data is pair 67 of the MLR \cite{}.
\begin{table*}
\begin{tabular}{l|cccc}
 $l$ & $\mathbf{\tau}_1$ & $\mathbf{\tau}_2$ & $\mathbf{\tau}_3$ & $\mathbf{\tau}_4$\\
\hline
 1 & 0.040 & 0.025 & 0.008 & 0.007 \\
 2 & 0.036 & -0.006 & -0.004 & 0.018 \\
 3 & 0.032 & 0.007 & -0.013 & 0.020 \\
 4 & 0.030 & 0.045 & -0.003 & 0.024 \\
 5 & 0.028 & -0.030 & -0.002 & 0.016 \\
 6 & 0.025 & 0.038 & -0.037 & 0.011 \\
 7 & 0.024 & 0.006 & -0.019 & 0.036 \\
 8 & 0.024 & 0.014 & -0.004 & 0.030 \\
 9 & 0.023 & 0.009 & -0.023 & 0.017 \\
 10 & 0.022 & -0.001 & -0.021 & 0.033 \\
 11 & 0.020 & -0.008 & -0.005 & 0.055 \\
 12 & 0.018 & 0.058 & 0.003 & 0.024 \\
 13 & 0.018 & 0.015 & -0.015 & 0.019 \\
 14 & 0.019 & 0.024 & -0.020 & 0.042 \\
 15 & 0.017 & 0.002 & -0.013 & 0.025\\
\end{tabular} 
\caption{The weighted mean observed leaning, $\lambda_l^{xy}$, using the $1$-standard cause-effect assignment for $l\in[0,15]$.  The time series pairs $\mathbf{\tau}_1 = (\mathbf{X},\mathbf{Y})$ with $\mathbf{X}$ and $\mathbf{Y}$ shown in Figure \ref{fig:empdata}(c) and (d), respectively, and $\mathbf{\tau}_2 = (\mathbf{X},\mathbf{Y})$ with $\mathbf{X}$ and $\mathbf{Y}$ shown in Figure \ref{fig:empdata}(a) and (b), respectively.  The last two time series pairs $\mathbf{\tau}_3$ and $\mathbf{\tau}_4$ are similar to $\mathbf{\tau}_2$ and are described in more detail in the text.  The expected causal inference for all the time series pairs is $\mathbf{X}\xrightarrow{lean}\mathbf{Y}$; i.e., the expectation is $\lambda_l^{xy} > 0$ for every entry in this table.}
\label{tab:emp}
\end{table*}

Table \ref{tab:emp} shows the leanings, using the given tolerance domains, imply causal inferences that agree with the causal truths for all the tested pairs with $1$-standard assignments.  However, only the $\mathbf{\tau}_1$ and $\mathbf{\tau}_2$ leanings agree with the causal truths with all the tested $l$-standard assignments.  The $\mathbf{\tau}_2$ leanings agree with the casual truth for most of the $l$-standard assignments. But, $\mathbf{\tau}_3$ only agrees with the truth for two of the tested $l$-standard assignments, despite its similarities with $\mathbf{\tau}_2$ and $\mathbf{\tau}_3$.

Estimating the tolerance domains with $\sigma_{\mathbf{X}-\langle\mathbf{X}\rangle}$ and $\sigma_{\mathbf{Y}-\langle\mathbf{Y}\rangle}$ causes the fewer negative values in set of $\mathbf{\tau}_3$ leanings shown in Table \ref{tab:emp}, but it also causes more negative values in the set of leanings shown for $\mathbf{\tau}_1$.  There are times series pairs in the MLR for which the leaning calculations using an $l$-standard assignment for $l\in[0,15]$ agree with the causal truth using either estimation technique for the tolerance domains.  For example, this statement is true for pair 72 \cite{UCI} where $\mathbf{X}$ is the monthly sunspot number for the years 1874 to 2010 \cite{NASA} and $\mathbf{Y}$ is the monthly global mean temperature anomalies for the same years \cite{both}.
...
\end{equation}
then for $\mathbf{\tau}_1$, $\lambda^{xy}_{max}=0.040\Rightarrow\mathbf{X}\xrightarrow{lean}\mathbf{Y}$, which agrees with the causal truth, for $\mathbf{\tau}_2$, $\lambda^{xy}_{max}=0.058\Rightarrow\mathbf{X}\xrightarrow{lean}\mathbf{Y}$, which agrees with the causal truth, for $\mathbf{\tau}_4$, $\lambda^{xy}_{max}=0.055\Rightarrow\mathbf{X}\xrightarrow{lean}\mathbf{Y}$, which agrees with the causal truth, and for $\mathbf{\tau}_3$, $\lambda^{xy}_{max}=-0.037\Rightarrow\mathbf{Y}\xrightarrow{lean}\mathbf{X}$, which does not agree with the causal truth.  Thus, the $l$-standard cause-effect assignment may not be the most appropriate cause-effect assignment for $\mathbf{\tau_3}$.
